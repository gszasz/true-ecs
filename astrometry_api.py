#!/usr/bin/env python3
"""Python client for the Astrometry.net service.
...
Based on the original code of the Astrometry.net client.
...
class Client(object):
    ...
    def send_request(self, service, args={}, file_args=None):
        ...
        if file_args is not None:
            import random
            boundary_key = ''.join([random.choice('0123456789')
                                    for i in range(19)])
    ...
    def retrieve_corrected_wcs(self, fn, wcsfn, **kwargs):
        ...
        # Wait until any spawned job can be selected
        while True:
            stat = self.sub_status(sub_id, justdict=True)
            print('Got status:', stat)
            jobs = stat.get('jobs', [])
            if len(jobs):
                for j in jobs:
        ...
        # Wait until selected job is finished with success
        while True:
            stat = self.job_status(solved_id, justdict=True)
...
if __name__ == '__main__':
    ...
    parser.add_option('--upload-xy', dest='upload_xy',
                      help='Upload a FITS x,y table as JSON')
    ...
    parser.add_option('--scale-units', dest='scale_units',
                      choices=('arcsecperpix', 'arcminwidth', 'degwidth',
                               'focalmm'),
                      help='Units for scale estimate ("arcsecperpix",'
                      ' "arcminwidth", "degwidth", or "focalmm")')
    # parser.add_option('--scale-type', dest='scale_type',
    #                   choices=('ul', 'ev'),
    #                   help='Scale bounds: lower/upper or estimate/error')
    ...
    parser.add_option('--scale-err', dest='scale_err', type=float,
                      help='Scale estimate error (in PERCENT), eg "10" if you'
                      ' estimate can be off by 10%')
    ...
    parser.add_option('--positional_error', dest='positional_error',
                      type=float,
                      help='How many pixels a star may be from where it should'
                      ' be.')
    ...
    parser.add_option('--invert', action='store_true', default=None,
                      help='Invert image before detecting sources -- for'
                      ' white-sky, black-stars images')
    parser.add_option('--image-width', type=int,
                      help='Set image width for x,y lists')
    parser.add_option('--image-height', type=int,
                      help='Set image height for x,y lists')
    parser.add_option('--album', type=str,
                      help='Add image to album with given title string')
    parser.add_option('--sdss', dest='sdss_wcs', nargs=2,
                      help='Plot SDSS image for the given WCS file; write plot'
                      ' to given PNG filename')
    ...
    parser.add_option('--jobid', '-i', dest='solved_id', type=int,
                      help='retrieve result for jobId instead of submitting'
                      ' new image')
    ...
    if opt.upload or opt.upload_url or opt.upload_xy:
        ...
        for key in ['scale_units', 'center_ra', 'center_dec', 'radius',
                    'downsample_factor', 'positional_error', 'tweak_order',
                    'crpix_center', 'album']:
    ...
    if opt.solved_id:
        ...
        if opt.newfits:
            url = opt.server.replace('/api/',
                                     '/new_fits_file/%i/' % opt.solved_id)
